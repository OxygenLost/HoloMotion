...
class PPO:
    def __init__(
        ...
        # Note: Variance penalty lambda is now configured directly in motion library config
    ...
    def _init_config(self):
        ...
        # TD-UCB curriculum learning configuration (unified)
        self.use_td_ucb_curriculum = self.config.get(
            "use_td_ucb_curriculum", False
        )
        # Note: All TD-UCB parameters (momentum, confidence, variance penalty)
        # are now configured in motion library config section

        logger.info(
            f"TD-UCB curriculum learning enabled: {self.use_td_ucb_curriculum} "
            f"(parameters configured in motion library)"
        )
    ...
    def _setup_models_and_optimizer(self):
        ...
        if self.use_accelerate and hasattr(self, "accelerator"):
            # Add synchronization barrier before model preparation
            logger.info(
                f"Process {self.process_rank}: Starting distributed model preparation..."
            )

            try:
                # Wait for all processes before starting model preparation
                logger.info(
                    f"Process {self.process_rank}: Waiting for all processes before model prep..."
                )
                self.accelerator.wait_for_everyone()

                # Prepare models one by one with barriers to avoid race conditions
                logger.info(
                    f"Process {self.process_rank}: Preparing actor model (this may take several minutes for large models)..."
                )
                self.actor, self.actor_optimizer = self.accelerator.prepare(
                    self.actor, self.actor_optimizer
                )
                logger.info(
                    f"Process {self.process_rank}: ✅ Actor preparation completed"
                )

                # Wait after actor preparation
                self.accelerator.wait_for_everyone()

                if not self.dagger_only:
                    logger.info(
                        f"Process {self.process_rank}: Preparing critic model..."
                    )
                    ...
                    logger.info(
                        f"Process {self.process_rank}: ✅ Critic preparation completed"
                    )

                    # Wait after critic preparation
                    self.accelerator.wait_for_everyone()

                    if self.use_amp:
                        logger.info(
                            f"Process {self.process_rank}: Preparing discriminator model..."
                        )
                        ...
                        logger.info(
                            f"Process {self.process_rank}: ✅ Discriminator preparation completed"
                        )
                        self.accelerator.wait_for_everyone()

                    if self.use_rnd:
                        logger.info(
                            f"Process {self.process_rank}: Preparing RND model..."
                        )
                        ...
                        logger.info(
                            f"Process {self.process_rank}: ✅ RND preparation completed"
                        )
                        self.accelerator.wait_for_everyone()

                # Final synchronization after all models are prepared
                logger.info(
                    f"Process {self.process_rank}: All models prepared successfully!"
                )
                self.accelerator.wait_for_everyone()
                logger.info(
                    f"Process {self.process_rank}: 🎉 Distributed model preparation phase completed!"
                )

            except Exception as e:
                logger.error(
                    f"Process {self.process_rank}: ❌ Model preparation failed: {e}"
                )
                # Try to gather error info from all processes
                if torch.distributed.is_initialized():
                    rank = torch.distributed.get_rank()
                    world_size = torch.distributed.get_world_size()
                    logger.error(
                        f"Process {rank}/{world_size}: Exception during model prep: {str(e)}"
                    )
                raise
    ...
    def _setup_storage(self):
        ...
        # Add motion ID tracking for TD-UCB curriculum learning
        if (
    ...
    def learn(self):
        ...
        for it in range(self.current_learning_iteration, tot_iter):
            ...
            # Global curriculum synchronization using accelerate APIs
            if (
                ...
                logger.info(
                    f"Synchronizing curriculum state globally at iter {it} using accelerate..."
                )
                if self.use_accelerate and hasattr(self, "accelerator"):
                    ...
                    logger.warning(
                        "Curriculum sync enabled but accelerate not available - skipping sync"
                    )
            ...
            if self.use_accelerate and hasattr(self, "accelerator"):
                ...
                if (
                    ...
                    try:
                        # Test gradient synchronization with accelerate reduce
                        test_tensor = torch.tensor(
                            float(self.process_rank), device=self.device
                        )
                        reduced_tensor = self.accelerator.reduce(
                            test_tensor, reduction="sum"
                        )

                        expected_sum = sum(
                            range(self.accelerator.num_processes)
                        )
                        actual_sum = reduced_tensor.item()
                        ...
                        if self.rnd_reward_norm:
                            ...
                            if (
                                ...
                                for buff_name in [
                                    ...
                                    reduced_buff = self.accelerator.reduce(
                                        buff, reduction="mean"
                                    )
                                    buff.copy_(reduced_buff)
    ...
    def _compute_returns(self, last_obs_dict, policy_state_dict):
        ...
        if td_errors is not None:
            ...
            if motion_ids_storage is not None:
                ...
                # Collect TD errors and calculate statistics per motion ID
    ...
    def _init_loss_dict_at_training_step(self):
        ...
        loss_dict["Actor_Domain_Est_Loss"] = 0
        loss_dict["Actor_Lin_Vel_Est_Loss"] = 0
        return loss_dict
    ...
    def _update_ppo(self, policy_state_dict, loss_dict):
        ...
        if self.actor_type == "EstVAEStudent":
            if self.use_accelerate and hasattr(self.actor, "module"):
                domain_est_loss = (
                    self.actor.module.actor_module.compute_domain_est_loss()
                )
            else:
                domain_est_loss = (
                    self.actor.actor_module.compute_domain_est_loss()
                )
            actor_loss = actor_loss + domain_est_loss
            loss_dict["Actor_Domain_Est_Loss"] += domain_est_loss.item()

            if self.use_accelerate and hasattr(self.actor, "module"):
                lin_vel_est_loss = (
                    self.actor.module.actor_module.compute_lin_vel_est_loss()
                )
            else:
                lin_vel_est_loss = (
                    self.actor.actor_module.compute_lin_vel_est_loss()
                )
            actor_loss = actor_loss + lin_vel_est_loss
            loss_dict["Actor_Lin_Vel_Est_Loss"] += lin_vel_est_loss.item()
    ...
    def _logging_to_writer(self, log_dict, train_log_dict, env_log_dict):
        ...
        if self.use_rnd:
            ...
            # Log TD-UCB curriculum learning statistics (unified)
        if self.use_td_ucb_curriculum:
            self.env._motion_lib.log_curriculum_to_tensorboard(
                self.tensorboard_writer, log_dict["it"]
            )
    ...
    def evaluate_policy(self, keyboard_commander=None):
        ...
        while not last_eval_batch:
            ...
            self.env._motion_lib.sample_global_start_frames(
                torch.arange(self.env.num_envs),
                eval=True,
            )
            logger.info(
                f"sampled global start frames: {self.env.motion_global_start_frame_ids}"
            )
