...
#         )
    ...
    def export_motion_clip_for_commands(self, motion_key: str) -> Dict[str, Union[int, np.ndarray, list]]:
        """Export a motion clip in the format expected by MotionCommand.

        Returns a dictionary with keys compatible with MotionLoader usage:
        - 'fps': int
        - 'joint_pos': np.ndarray [T, num_dofs]
        - 'joint_vel': np.ndarray [T, num_dofs]
        - 'body_pos_w': np.ndarray [T, num_bodies, 3]
        - 'body_quat_w': np.ndarray [T, num_bodies, 4]
        - 'body_lin_vel_w': np.ndarray [T, num_bodies, 3]
        - 'body_ang_vel_w': np.ndarray [T, num_bodies, 3]
        """
        raw = self.export_motion_clip(motion_key)
        if not raw:
            return {}
        ...
        for key in required:
            if key not in raw:
                raise ValueError(f"Missing '{key}' in exported motion clip for key {motion_key}")
            
        motion_lib_dof_order = [
        ...
        return {
            "motion_lib_dof_order": motion_lib_dof_order,
            "motion_lib_body_order": motion_lib_body_order,
            "fps": int(raw["fps"]),
            "joint_pos": raw["dof_pos"],
            "joint_vel": raw["dof_vel"],
            "body_pos_w": raw["rg_pos"],
            "body_quat_w": raw["rb_rot"],
            "body_lin_vel_w": raw["body_vel"],
            "body_ang_vel_w": raw["body_ang_vel"],
        }
